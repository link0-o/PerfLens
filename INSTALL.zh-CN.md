...
```text
perflens-0.3.2-py3-none-any.whl
```

`.whl` 是 Python 安装包，不是需要解压后点击运行的 ZIP。不要提取它。解压后看到的 `perflens/` 和 `perflens-0.3.2.dist-info/` 只是程序模块与安装元数据。

其他 Release 文件的用途：

- `perflens_0.3.2-1_amd64.deb`：Debian 13 普通用户主安装包；
- `perflens-collector_0.3.2-1_amd64.deb`：可选 Collector 与 Rust Helper，需配合同版本主包；
- `perflens-0.3.2.tar.gz`：源码发行包；
- `perflens-skill-0.3.2.zip`：只包含 Agent Skill；

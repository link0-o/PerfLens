...
如果系统没有部署 Collector，则项目初始化回退到 `cap_perfmon`，但不会部署或启动它。如果系统策略存在却所有者、权限、大小或内容不安全，初始化必须失败并提示修复，不能静默猜测。

高级用户仍可使用 `--collector-privilege-mode` 显式准备某种模式的部署资产。显式值与已部署模式冲突时，普通初始化必须报错；带 `--prepare-collector` 时允许生成候选资产，但这不等于切换系统服务。

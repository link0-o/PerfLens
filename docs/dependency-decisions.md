...
| Active collection | Reuse system perf | Supported, default-off wrappers cover `record` and `stat`. The `cap_perfmon` Broker retains disabled raw `sched`/`lock`/`off_cpu` experiments, but dedicated deterministic analyzers must precede any stable claim. PerfLens owns authorization, bounds, diagnostics, and immutable output, not kernel instrumentation. |

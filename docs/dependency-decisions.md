...
| Active collection | Reuse system perf | `record`/`stat` are stable CPU modes. The opt-in `full_diagnostics` profile uses a separate target-filtered Trace Helper plus deterministic `sched`/`off_cpu`/`lock` analyzers; incomplete evidence remains `partial`. PerfLens owns authorization, bounds, diagnostics, and immutable output, not kernel instrumentation. |

# PerfLens Collector 与用户态锁能力路线图（v0.3.0 / v0.4.0）
...
5. **`v0.4.0` 增加四类正式用户态锁 Adapter。** 覆盖 C/C++ pthread、Java JFR、
...
这些参数随 v0.3.0 提供；`0.2.0` 安装包没有这些接口。
...
## 7. v0.4.0 用户态锁 Adapter

### 7.1 “完整覆盖”的产品含义

用户态锁不是做不到，而是不存在一个通用 perf 命令一次覆盖所有语言和实现。`v0.4.0`
...
`v0.4.0` 只有在四类 Adapter、统一 verifier、安全拒绝路径、真实运行时矩阵，以及两个
DEB 的安装、升级、回滚和卸载测试全部通过后才能标记稳定。缺少任一 Adapter 时必须缩小
发布声明或推迟版本，不能仍使用“四类正式覆盖”的名称。

## 9. 发布和兼容表述

- v0.3.0 只以完整、可选的五模式配置发布 `full_diagnostics`；任一模式在当前主机验收
  失败时必须使用 `cpu_only`，不能以完整诊断名义部署部分能力。
- 任何一个高级模式未通过门禁时，不能用“完整诊断”名称掩盖缺失能力。
- `v0.3.1` 只表示 Docker 进程支持，其边界以 Docker 路线图为准。
- `v0.4.0` 的四类 Adapter 必须分别列出准确支持版本、后端、采样语义和不可见快路径。

...
| Python 3.13 | isolated `pytest -q` environment | 267 passed |
...
| Coverage | `pytest --cov=perflens --cov-fail-under=85` | 85.68%, passed |
...
| Native DEB | split Debian 13 main/Collector packages | extracted entry-point/config smoke and byte-for-byte rebuild passed locally; installed deploy preflight is also enforced in CI |

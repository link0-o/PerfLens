...
| wheel/sdist | two commit-timestamped builds + byte comparison + isolated installation | reproducible and passed |
| Release provenance | isolated pinned `actions/attest` job → SLSA provenance → gated publisher | configured and regression-tested; first remote tag run remains required |
...
CLI/MCP/Skill/Collector smoke output, deterministic Python/DEB hashes, and the
expected release artifact hashes also passed in fresh local temporary
...
not be moved or overwritten. The current candidate is `v0.2.0`; after the release
commit is pushed, create the new `v0.2.0` tag.

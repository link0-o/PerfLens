...
输入被视为不可变数据。普通分析永远不能覆盖源 Profile。产物先写入 `0600` 同目录
临时文件，完成序列化和文件 `fsync` 后，通过固定目录描述符原子发布；随后同步目录项，
并复核目录路径仍指向同一 inode。写入器新建的多级父目录也会逐级同步。主动采集只能
发布到不存在的新路径。
...
MCP ArtifactStore 按产物 ID 采用只追加语义：发布时不替换路径，相同字节视为幂等，
同 ID 不同内容直接拒绝。读取会固定 artifact root 身份，并用非阻塞、不跟随软链接的
描述符，只接受未变化、单链接、`0600` 的普通文件。这能阻止 FIFO 卡死和路径替换被
当成有效证据，但不能把同 UID 的恶意项目程序变成已隔离进程。
...
重放；合法墓碑与性能证据分开计数和归档，过期墓碑按策略最长 TTL 有界回收，异常墓碑
则让采集和管理员证据操作安全失败。

收到成功响应后，普通用户客户端仍会独立校验由计划 ID 推导的证据文件名，并以不跟随
软链接的方式打开文件。只有服务属主、Collector Socket 同组、单链接普通文件、授权组
只读、读取前后身份不变、大小未超计划且 SHA-256 与声明一致时，产物才会被接受。
...
项目工作负载使用 Broker 协议 `1.1` 的流式就绪帧，不再依赖固定延时。普通用户引导进程
先保持同一 PID 等待；`auto` 硬件 probe 期间不发送就绪，零/不可用计数会保守选择软件。
最终选中的正式 perf 以禁用事件启动，控制通道确认绑定后
再次核验 PID/UID/启动时间，启用事件，再返回绑定 `request_id`、计划和 PID 的
`collection_ready`。客户端认证该帧后才让引导进程 `exec` 用户确认的程序。高级模式继续
转发到 Helper 私有协议 `1.2`，Helper 会独立执行相同顺序；任一错配都不会放行程序。
...
项目 MCP 配置还必须让入口程序重新通过存在性、执行权限和可信路径检查后才算就绪。

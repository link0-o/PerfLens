...
状态：**已随 v0.3.1 发布；每台安装主机仍须独立完成显式验收**

最后审计：2026-08-26，基于已发布的 v0.3.1 边界和 v0.3.2 兼容路径

发布版本：`v0.3.1`

本文是 v0.3.1 的设计、实现、使用和发布合同。已发布路径实现下述有界 Docker 范围内的
主动发现、授权、托管启动、采集、分析和匹配比较。每台主机仍须
...
在 v0.3.1 固定镜像工作流中，修改后的代码通过只读 `/workspace` 和私有 scratch 进入相同
工具链镜像。Python 等解释型负载可直接运行；编译型负载可以在固定容器命令中把产物
写入 scratch，或使用用户在宿主机生成并位于项目根内的构建产物。重新构建这个固定镜像会
改变 digest，因此需要新的 v0.3.1 授权。v0.3.2 已另行提供默认关闭、绑定固定构建配方的
授权；其中经过验证的 baseline/candidate 镜像变化属于合法 Treatment，但这并不把任意镜像
变化视为已授权。详见[《v0.3.2 有界 Docker 自动优化合同》](docker-optimization-roadmap.zh-CN.md)。
...
这次确认必须发生在展示完整、已经解析的会话摘要之后，并由用户在下一条消息中明确回复。
MCP 权限弹窗、持久工具允许列表、Agent 自动批准模式、最初的自然语言优化请求，或模型自行
填写固定授权字符串，都不能替代这次确认。
